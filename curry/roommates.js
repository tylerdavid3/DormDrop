...
(function initSchoolContext() {
  ...
  } else if (p.indexOf('/curry') !== -1) {
    SCHOOL_KEY = 'curry';
    SCHOOL_NAME = 'Curry College';
    EMAIL_SUFFIX = '@curry.edu';
  } else {
    SCHOOL_KEY = 'bu';
...
function renderLoggedOutState() {
  ...
  if (pg)
    pg.innerHTML =
      ...
      '<p class="rm-cta-desc">Create a free DormDrop account to take the lifestyle quiz and see compatible BU students with built-in messaging. We match on sleep schedule, study habits, cleanliness, and more — not just proximity.</p>' +
      '<ul class="rm-cta-bullets">' +
      '<li><span class="rm-cta-check">&#10003;</span> Match with verified BU students only</li>' +
...
// ── Interest Modal (polished alternative to contactListing prompt) ─────────────
  ...
  db.collection('interests')
    ...
    .then(function(snap) {
      ...
      var data = {
        ...
        studentEmail:   currentUser.email,
        studentSchool:  SCHOOL_NAME || 'Boston University',

...
function showAuthView(viewId) {
  ...
  var titles = {
    signupFields: { t: 'Create your account',  s: 'Free for all BU students. No credit card required.' },
    loginFields:  { t: 'Welcome back',          s: 'Sign in to your DormDrop account.' },
    forgotFields: { t: 'Reset password',        s: 'We\'ll send a reset link to your BU email.' },
...
function doSignup() {
  ...
  if (!email.toLowerCase().endsWith('@bu.edu')) { showAuthError('Please use your @bu.edu email address.'); return; }
...
function renderLoggedOutState() {
  ...
  if (pg) pg.innerHTML =
    '<div class="rm-state-box">' +
      ...
      '<p class="rm-state-desc">Create a free account to see BU students who match your lifestyle.</p>' +
...
function loadMatches() {
  ...
  db.collection('users')
    .where('school', '==', 'Boston University')
    .limit(200)
    .get()
    .then(function (snap) {
      var matches = [];
  ...
  if (matches.length === 0) {
    pg.innerHTML =
      '<div class="rm-state-box">' +
        ...
        '<p class="rm-state-desc">Be among the first! As more BU students join, your matches will appear here.</p>' +
      '</div>';
    return;
  }
...
function openMatchDetail(uid) {
  ...
  db.collection('users').doc(uid).get().then(function (snap) {
    ...
    document.getElementById('matchDetailContent').innerHTML =
      ...
      '</div>' +
      '<div class="d-addr">' + esc(p.year || 'BU Student') + ' &middot; Boston University' +
